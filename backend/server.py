...
class Settings(BaseSettings):
    gemini_api_key: str; mongo_uri: str; admin_api_key: str = Field(..., alias='ADMIN_API_KEY'); ip_geolocation_api_key: Optional[str] = Field(None, alias='IP_GEOLOCATION_API_KEY')
    model_config = SettingsConfigDict(env_file='.env', extra='ignore')

# --- PYDANTIC MODELS ---
class SagaResponse(BaseModel):
    prophecy_type: str; data: Any

class CommerceAuditRequest(BaseModel):
    prophecy_type: Literal["Commerce Audit"] = Field("Commerce Audit"); audit_type: Literal["Account Audit", "Store Audit", "Account Prediction"]; statement_text: Optional[str] = None; store_url: Optional[str] = None
class ArbitragePathsRequest(BaseModel):
    prophecy_type: Literal["Arbitrage Paths"] = Field("Arbitrage Paths"); mode: Literal["User_Buys_User_Sells", "Saga_Buys_User_Sells", "User_Buys_Saga_Sells", "Saga_Buys_Saga_Sells"]; product_name: Optional[str] = None; buy_from_url: Optional[str] = None; sell_on_url: Optional[str] = None; category: Optional[str] = None; subcategory: Optional[str] = None
class SocialSellingSagaRequest(BaseModel):
    prophecy_type: Literal["Social Selling Saga"] = Field("Social Selling Saga"); product_name: str; social_selling_price: float; desired_profit_per_product: float; social_platform: str = "Instagram"; ads_daily_budget: float = 10.0; product_category: Optional[str] = None; product_subcategory: Optional[str] = None
class ProductRouteRequest(BaseModel):
    prophecy_type: Literal["Product Route"] = Field("Product Route"); location_type: Literal["Global", "My Location"]
...
class GrandStrategyRequest(BaseModel):
    interest: str; sub_niche: Optional[str] = None; user_content_text: Optional[str] = None; user_content_url: Optional[str] = None; target_country_name: Optional[str] = None; asset_info: Optional[AssetInfo] = None
...
class NewVentureRequest(BaseModel):
    interest: str; sub_niche: Optional[str] = None; user_content_text: Optional[str] = None; user_content_url: Optional[str] = None; target_country_name: Optional[str] = None; venture_brief: Optional[VentureBrief] = None

class NewVentureBlueprintRequest(BaseModel):
    venture_session_id: str; vision_id: str
...
class MarketingAssetRequest(BaseModel):
    marketing_session_id: str; angle_id: str; platform: Optional[str] = None; length: Optional[str] = None
...
class GrimoirePageUpdate(BaseModel):
    title: Optional[str] = None; slug: Optional[str] = None; content: Optional[str] = None; summary: Optional[str] = None; tags: Optional[List[str]] = None
...
engine: SagaEngine = None; scout: MarketplaceScout = None; settings = Settings()
...
async def discover_marketplaces_endpoint(background_tasks: BackgroundTasks):
    if not scout: raise HTTPException(status_code=503, detail="Scout not ready.")
    background_tasks.add_task(scout.find_general_marketplaces); return {"status": "Discovery launched."}
...
async def get_commerce_prophecy(request: CommerceAuditRequest | ArbitragePathsRequest | SocialSellingSagaRequest | ProductRouteRequest):
    ...
    except Exception as e: logger.error(f"Error: {e}"); raise HTTPException(status_code=500, detail="Error.")
...
async def create_grimoire_page(page: GrimoirePageCreate, db: motor.motor_asyncio.AsyncIOMotorDatabase = Depends(get_database)):
    ...
    if await db.grimoire_pages.find_one({"slug": page_dict["slug"]}): raise HTTPException(status_code=400, detail="Slug exists.")
...
async def update_grimoire_page(id: str, page_update: GrimoirePageUpdate, db: motor.motor_asyncio.AsyncIOMotorDatabase = Depends(get_database)):
    ...
    raise HTTPException(status_code=404, detail=f"ID not updated.")
...
async def delete_grimoire_page(id: str, db: motor.motor_asyncio.AsyncIOMotorDatabase = Depends(get_database)):
    if (await db.grimoire_pages.delete_one({"_id": ObjectId(id)})).deleted_count == 0:
...
async def get_all_grimoire_pages(db: motor.motor_asyncio.AsyncIOMotorDatabase = Depends(get_database)):
    pages = await db.grimoire_pages.find().sort("created_at", -1).to_list(length=100)
...
async def get_grimoire_page_by_slug(slug: str, db: motor.motor_asyncio.AsyncIOMotorDatabase = Depends(get_database)):
    if (page := await db.grimoire_pages.find_one({"slug": slug})): return GrimoirePageDB.model_validate(page)
...
async def startup_event():
    ...
    if settings.gemini_api_key:
        ...
        logger.info("Saga's Engine, Scout, and Memory Scrolls are awake.")
    else: logger.error("GEMINI_API_KEY not found. Saga cannot awaken.")
